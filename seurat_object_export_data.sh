...
function show_help() {
  echo "Usage: $0 <path_to_rds_file> <format> <data_type> <filename_prefix> <barcodes_file>"
  ...
  echo "  <barcodes_file>     : File containing barcodes to filter output (optional)."
...
# Load barcodes from the file if provided
if ('$BARCODES_FILE' != '') {
  barcodes <- read.table('$BARCODES_FILE', header=FALSE, stringsAsFactors=FALSE)[,1]
  cat('Barcodes loaded from $BARCODES_FILE\n')
  sobj <- subset(sobj, cells = barcodes)
}
...
if ('$FORMAT' == 'full') {
  ...
  # Export features (genes) in the correct format
